...
public class FractionalKnapsack {
    public static void FractionalKnapsackSolve(int n, int[] p, int[] w, int W) {
        int i, j;
        int weightUsed = 0;
        int TotalProfit = 0;
        ...
        // Sorting according to profit by weight in descending order
        for(i=0;i<n-1;i++){
            for(j=0;j<n-i-1;j++){
                if(pbyw[j] < pbyw[j+1]){
                    double temp = pbyw[j];
        ...
        System.out.println("Total weight occupied is" +weightUsed);
    }
    public static void main(String[] args) {
        ...
        int W = 60;
        int n = p.length;
        FractionalKnapsackSolve(n, p, w, W);
    }
}
